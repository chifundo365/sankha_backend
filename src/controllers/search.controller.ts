...
export const search = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const radiusKm = req.query.radius_km ? Math.min(Math.max(Number(req.query.radius_km) || 15, 1), 500) : 15;
    ...
    const runSearchQuery = (radiusFilter: Prisma.Sql) => prisma.$queryRaw<any[]>`
...
`;
    ...
    // First attempt: search within radius
    let rawRows: any = await runSearchQuery(withinRadiusFilter);
    let radiusExpanded = false;

    // Fallback: if buyer has coords but no results within radius, expand to all locations
    if (buyerHasCoords && Array.isArray(rawRows) && rawRows.length === 0) {
      rawRows = await runSearchQuery(Prisma.sql`TRUE`);
      radiusExpanded = true;
    }
    ...
    const metadata = {
      ...
      currency: 'MWK', buyer_location_provided: buyerHasCoords,
      search_radius_km: buyerHasCoords ? radiusKm : null,
    ...
    const logPage = page ?? 1;
    const logLimit = limit ?? 20;
    const logResponseTime = responseTime ?? 0;
    const logResultsCount = totalCount ?? 0;
    prisma.$executeRaw`
      INSERT INTO search_logs (query, results_count, filters, buyer_has_coords, response_time_ms, page, limit_per_page)
      VALUES (${qRaw}, ${logResultsCount}::int, ${JSON.stringify(filtersObj)}::jsonb, ${buyerHasCoords}, ${logResponseTime}::int, ${logPage}::int, ${logLimit}::int)
