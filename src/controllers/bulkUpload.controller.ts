...
export const bulkUploadController = {
  ...
  /**
   * Upload products via Excel file (v4.0 - Staging System)
  ...
  bulkUpload: async (req: Request, res: Response) => {
    try {
      ...
      const parseResult = bulkUploadService.parseExcelFile(file.buffer);

      // Convert v3 format to v4 format (camelCase)
      const rows = parseResult.rows.map(row => ({
        ...
        displayPrice: calculateDisplayPrice(row.base_price),
        stockQuantity: row.stock_quantity,
        condition: row.condition,
        description: row.shop_description,
        variantValues: row.specs || {},
      ...
      // v4.0: Create staging batch with full tech spec validation
      ...
      if (autoCommit) {
        ...
        return successResponse(
          ...
          `Bulk upload completed: ${commitResult.committed} created, ${commitResult.skipped} skipped, ${commitResult.failed} failed`,
          {
            ...
            next_steps: commitResult.committed > 0 ? {
              message: 'Products created. Check listing status - products may need images or tech specs.',
              needs_images_endpoint: `/api/shops/${shopId}/products/needs-images`,
              needs_specs_endpoint: `/api/shops/${shopId}/products/needs-specs`
            } : undefined
          },
          201
        );
      ...
      // Return staging preview (default v4.0 behavior - get preview)
      const preview = await bulkUploadStagingService.getPreview(shopId, batchId, 1, false);
      
      return successResponse(
        res,
        'File validated successfully. Review the preview and commit when ready.',
        {
          batch_id: batchId,
          upload_id: bulkUpload.id,
          ...
          preview: preview,
          next_steps: {
            message: 'Review the preview and commit the batch when ready',
            preview_endpoint: `/api/shops/${shopId}/products/bulk/${batchId}/preview`,
            commit_endpoint: `/api/shops/${shopId}/products/bulk/${batchId}/commit`
          }
        },
        200
      );
